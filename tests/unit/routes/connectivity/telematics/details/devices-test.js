...
module('Unit | Route | connectivity/telematics/details/devices', function (hooks) {
    setupTest(hooks);

    test('it exists', function (assert) {
        let route = this.owner.lookup('route:connectivity/telematics/details/devices');
        assert.ok(route);
    });

    test('device filter query params refresh the model', function (assert) {
        const route = this.owner.lookup('route:connectivity/telematics/details/devices');
