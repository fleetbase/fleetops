...
test('vehicle controller keeps the odometer in the input it builds from a request', function () {
    ...
    $controller = new FleetOpsVehicleControllerProbe();
    $request    = new Request(['odometer' => 211098, 'odometer_unit' => 'km', 'plate_number' => 'SG-1234']);

    $input = $controller->callHelper('vehicleInputFromRequest', $request);
...
test('vehicle controller input still drops fields that are not part of the contract', function () {
    // The projection is a allowlist and must stay one — adding odometer must
    // not turn it into "whatever the caller sent".
    $controller = new FleetOpsVehicleControllerProbe();
    $request    = new Request(['odometer' => 1, 'company_uuid' => 'someone-elses-company', 'uuid' => 'forged']);
